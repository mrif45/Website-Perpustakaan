...
if (strlen($_SESSION['alogin']) == 0) {
    ...
    if (isset($_POST['return'])) {
        ...
        $denda = $_POST['denda'];
        $rstatus = 1;
        $sql = "update peminjaman set denda=:denda,status=:rstatus where id_pinjam=:rid";
        $query = $dbh->prepare($sql);
        $query->bindParam(':rid', $rid, PDO::PARAM_STR);
        $query->bindParam(':denda', $denda, PDO::PARAM_STR);
        $query->bindParam(':rstatus', $rstatus, PDO::PARAM_STR);
        $query->execute();

        $_SESSION['msg'] = "Info peminjaman buku telah diperbarui";
                                ...
                                if ($query->rowCount() > 0) {
                                    foreach ($results as $result) { ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <?php
                                            if ($result->denda == "") { ?>
                                                <input class="form-control" type="text" name="denda" id="denda" required />
